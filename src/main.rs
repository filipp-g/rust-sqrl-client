...
fn main() {
    let iuk: Digest = create_iuk(); //256 bit value, generated by using sha256 encryption on a randomly selected 32 bit value
    println!("{:?}", iuk);
    let imk: Digest = sqrl_enhash(iuk);  //identity masterkey is essentially an encrypted version of the iuk
    println!("{:?}", imk);

	let key_pair = create_keypair(imk, parse_domain(TEST_URL));   //per-site public/private keys derived from the imk and the domain from the sqrl URL
	println!("{:?}", key_pair);
...
//using the derived per site private key, creates a signature to be appended to all http queries with the SQRL server
//the SQRL server can verify this signature using the paired public key (user identity) sent with the request 
